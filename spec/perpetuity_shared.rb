...
shared_examples Perpetuity do |mappers|
  describe 'mapper generation' do
    it 'generates mappers' do
      mappers.generate_mapper_for Object
      mappers[Object].should be_a Perpetuity::Mapper
    end

    it 'provides a DSL within the generated mapper' do
      mappers.generate_mapper_for Object do
        id { object_id + 1 }
        attribute :object_id
      end
  ...
  describe 'persistence' do
    it "persists an object" do
      ...
      expect { mappers[Article].insert article }.
        to change { mappers[Article].count }.by 1
      mappers[Article].find(article.id).title.should eq 'I have a title'
    end

    it 'returns the id of the persisted object' do
      article = Article.new
      mappers[Article].insert(article).should eq article.id
    end

    it "gives an id to objects" do
      article = Article.new
      mappers[Article].give_id_to article, 1
    ...
    describe 'id injection' do
      ...
      it 'assigns an id to the inserted object' do
        mappers[Article].insert article
        article.should respond_to :id
      end

      it "assigns an id using Mapper.first" do
        mappers[Article].first.should respond_to :id
      end

      it 'assigns an id using Mapper.retrieve.first' do
        mappers[Article].retrieve.first.should respond_to :id
      end

      it 'assigns an id using Mapper.all.first' do
        mappers[Article].all.first.should respond_to :id
      end
    end
    ...
    describe 'persisting arrays' do
      ...
      it 'persists arrays' do
        ...
        mappers[Article].insert article
        mappers[Article].find(article.id).comments.should eq [1, 2, 3]
      end
      ...
      it 'persists arrays with unserializable objects in them' do
        ...
        mappers[Article].insert article
        mappers[Article].find(article.id).comments.first.tap do |persisted_comment|
  ...
  describe "deletion" do
    it 'deletes an object' do
      2.times { mappers[Article].insert Article.new }
      expect { mappers[Article].delete mappers[Article].first }.to change { mappers[Article].count }.by(-1)
    end

    it 'deletes an object with a given id' do
      article_id = mappers[Article].insert Article.new
      expect {
        mappers[Article].delete article_id
      }.to change { mappers[Article].count }.by(-1)
    end
    
    describe "#delete_all" do
      it "should delete all objects of a certain class" do
        mappers[Article].insert Article.new
        mappers[Article].delete_all
        mappers[Article].count.should eq 0
      end
    end
  end
  ...
  describe "retrieval" do
    it "gets all the objects of a class" do
      expect { mappers[Article].insert Article.new }.
        to change { mappers[Article].all.count }.by 1
    end
    ...
    it "returns a Perpetuity::Retrieval object" do
      mappers[Article].retrieve(id: 1).should be_an_instance_of Perpetuity::Retrieval
    end

    it "gets an item with a specific ID" do
      article = Article.new
      mappers[Article].insert article
      retrieved = mappers[Article].find(article.id)
    ...
    it "gets an item by its attributes" do
      article = Article.new
      mappers[Article].insert article
      retrieved = mappers[Article].retrieve(title: article.title)
    ...
    describe "Array-like syntax" do
      ...
      before do
        mappers[Article].insert draft
        mappers[Article].insert published
      end

      it 'selects objects using equality' do
        selected = mappers[Article].select { |article| article.title == 'Published' }
      ...
      it 'selects objects using greater-than' do
        selected = mappers[Article].select { |article| article.published_at < Time.now }
        ids = selected.map(&:id)
      ...
      it 'selects objects using greater-than-or-equal' do
        selected = mappers[Article].select { |article| article.views >= 3 }
        ids = selected.map(&:id)
        ids.should include published.id
        ids.should_not include draft.id
      end
      ...
      it 'selects objects using less-than-or-equal' do
        selected = mappers[Article].select { |article| article.views <= 0 }
        ids = selected.map(&:id)
        ids.should include draft.id
        ids.should_not include published.id
      end
      ...
      it 'selects objects using inequality' do
        selected = mappers[Article].select { |article| article.title.not_equal? 'Draft' }
        ids = selected.map(&:id)
        ids.should_not include draft.id
        ids.should include published.id
      end
      ...
      it 'selects objects using regular expressions' do
        selected = mappers[Article].select { |article| article.title =~ /Pub/ }
        ids = selected.map(&:id)
        ids.should include published.id
        ids.should_not include draft.id
      end
      ...
      it 'selects objects using inclusion' do
        selected = mappers[Article].select { |article| article.title.in %w( Published ) }
        ids = selected.map(&:id)
        ids.should include published.id
        ids.should_not include draft.id
      end
  ...
  describe 'pagination' do
    it 'specifies the page we want' do
      mappers[Article].retrieve.should respond_to :page
    end

    it 'specify the quantity per page' do
      mappers[Article].retrieve.should respond_to :per_page
    end

    it 'returns an empty set when there is no data for that page' do
      mapper = mappers[Article]
      mapper.delete_all
    ...
    it 'specifies per-page quantity' do
      mappers[Article].delete_all
      5.times { |i| mappers[Article].insert Article.new i }
      data = mappers[Article].retrieve.page(3).per_page(2)
  ...
  describe 'associations with other objects' do
    let(:user) { User.new }
    let(:topic) { Topic.new }
    let(:user_mapper) { mappers[User] }
    let(:topic_mapper) { mappers[Topic] }
  ...
  it 'uses accessor methods to read/write data' do
    ...
    mappers[Message].insert message
    saved_message = mappers[Message].find(message.id)
